...
def init_MP3_playlist(mode = 0):
    ...
    if (mode == 0):
        if (globals.MP3_Filter == ''):
            ...
            for filter in re.split('\|', globals.MP3_Filter):   # split list of filters, vertical bar separator
                mpc.filter(filter.strip()) # .filter function searches and adds, so the multiclause filter is conjunctive
...
def play_mp3(mode = 0):
    ...
    while (CM == globals.ClockMode):
        ...
        if (mode == 0):  # MP3 with a running clock
            dt = datetime.now() - d0
            min, sec = divmod(int(dt.total_seconds()), 60)
            tstr = "{0:02d}{1:02d}".format(min, sec)
            print_time(tstr)
        if (msg != msg_ant):
            td.msg = msg
            msg_ant = msg
